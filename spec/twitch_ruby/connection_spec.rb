require 'rspec'
require 'twitch_ruby'

describe TwitchRuby::Connection do
  # after do
  #   WebMock.reset!
  # end
  let(:connection) { double :connection, get: nil }
  before do
    allow(connection).to receive(:get).with("/stats").and_return('some data')
    Judge.connection = connection
  end
  
  describe '#get' do
    it 'raises Arugment error if no path is specified' do
      expect {
        c = TwitchRuby::Connection.new('client_id')
        content = c.get
      }.to raise_error(ArgumentError)
    end

    it 'raises FormatError if response is not valid JSON' do
      stub_request(:any, /.*/).to_return(:body => '"Invalid JSON')

      expect {
        c = TwitchRuby::Connection.new('client_id')
        json = c.get('/test')
      }.to raise_error(Twitch::Error::FormatError)
    end

    it 'raises FormatError with request URL' do
      status = 200
      body = '"Invalid JSON'

      stub_request(:any, /.*/).to_return(:status => status, :body => body)

      error = false
      begin
        c = TwitchRuby::Connection.new('client_id')
        json = c.get('/test')
      rescue TwitchRuby::Error::FormatError => e
        error = true
        expect(e.url).to match(/test/) 
        expect(e.status).to eq(status)
        expect(e.body).to eq(body)
      end

      expect(error).to be_truthy
    end

    it 'raises ClientError when HTTP status is 404' do
      status = 404
      body = 'Not found.'

      stub_request(:any, /.*/).to_return(:status => status, :body => body)

      error = false
      begin
        c = TwitchRuby::Connection.new('client_id')
        json = c.get('/test')
      rescue TwitchRuby::Error::ClientError => e
        error = true
        expect(e.url).to match(/test/)
        expect(e.status).to eq(status)
        expect(e.body).to eq(body)
      end

      expect(error).to be_truthy
    end

    it 'raises ServerError when HTTP status is 500' do
      status = 500
      body = 'Not found.'

      stub_request(:any, /.*/).to_return(:status => status, :body => body)

      error = false
      begin
        c = TwitchRuby::Connection.new('client_id')
        json = c.get('/test')
      rescue TwitchRuby::Error::ClientError => e
        error = true
        expect(e.url).to match(/test/)
        expect(e.status).to eq(status)
        expect(e.body).to eq(body)
      end

      expect(error).to be_truthy
    end
  end
end
